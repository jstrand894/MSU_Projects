...
WSS infested stems often contained larvae burrowing through multiple nodes. In 2022, 57.1% of stems were observed to have at least 4 nodes burrowed through, or 5 internodes occupied. In 2023, 62.3% of stems had at least 4 nodes infested [Fig. 4](#figure4)
...
weekly_averages %>%
  ...
  geom_point(aes(x = new_date, y = prcp)) +
  scale_x_date(
    date_breaks = "1 month",
    date_labels = "%B",
    expand = c(0,0),
...
lochy_precip %>%
  ...
  geom_point(aes(x = new_date, y = prcp),
             size = 1.5) +
  geom_line(aes(x = new_date, y = pmax(prcp, 0)),
            color = "black",
            linewidth = 0.5) +
  scale_x_date(
    date_breaks = "1 month",
...
ggplot() +
  geom_point(data = line_table_average_month,
             aes(x = new_date, y = prcp),
             size = 1.5) +
  geom_segment(aes(x = as.Date("0000-03-30", 
                               format = "%Y-%m-%d"), 
                   xend = as.Date("0000-07-15", 
                                  format = "%Y-%m-%d"),
                   y = category_averages %>%
                     filter(model_category == "no") %>%
                     pull(prcp), 
                   yend = category_averages %>%
                     filter(model_category == "no") %>%
                     pull(prcp)),
               color = "red",
               alpha = 0.5) +
  geom_rect(aes(xmin = as.Date("0000-03-30", 
                               format = "%Y-%m-%d"), 
  ...
  geom_rect(aes(xmin = as.Date("0000-07-15", 
            ...
            alpha = red_alpha,
            fill = "blue") +
  geom_line(data = line_table_average_month,
            aes(x = new_date, y = pmax(prcp, 0)),
            color = "black",
            linewidth = 1) +
  scale_x_date(
    date_breaks = "1 month",
...
# need to scrape another month on either end of each year. 
# make lines across entire plot showing averages (may to july, september, october)
# no smoothing
